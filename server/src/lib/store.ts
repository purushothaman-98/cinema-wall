...
export function getComments(): CommentRow[] {
  return loadCsv<Record<string, unknown>>(FILES.comments).map((row) => ({
...
export function getVideoSnapshots(): VideoSnapshotRow[] {
  return loadCsv<Record<string, unknown>>(FILES.videoSnapshots).map((row) => ({
    video_id: str(row.video_id),
    channel: str(row.channel, "Unknown"),
    title: str(row.title),
...
export function getChannelEvaluation(): ChannelEvaluationRow[] {
  return loadCsv<Record<string, unknown>>(FILES.channelEvaluation).map((row) => ({
    channel: str(row.channel, "Unknown"),
...
export function getTopChannelCoverage(): TopChannelCoverageRow[] {
  return loadCsv<Record<string, unknown>>(FILES.topChannelCoverage).map((row) => ({
    film: str(row.film, "Unknown"),
    channel: str(row.channel, "Unknown"),
...
export function getMetadata(): ScanMetadata | null {
  return loadJson<ScanMetadata>(FILES.metadata);
}
